...
class Adyen
{
    ...
    private function getUrl()
    {
        ...
        if ($this->type == 'moto') {
            ...
            if ($this->getIssuerId()) {
                return $this->live ? 'https://live.adyen.com/hpp/skipDetails.shtml' : 'https://test.adyen.com/hpp/skipDetails.shtml';
            }
    ...
    /**
     * Set the issuerId for direct redirects to the Payment Provider IssuerId.
     *
     * @param int $issuerId
     *
     * @return $this
    ...
    private function getHPPParams()
    {
        ...
        if (! $merchantReference) {
            throw new AdyenException('No merchantReference set.');
        }
        if (! $this->zero_auth_payments and ! $paymentAmount) {
            throw new AdyenException('Invalid paymentAmount: zero.');
        }
        if (! $shipBeforeDate) {
            throw new AdyenException('No shipBeforeDate set.');
        }
        if (! $skinCode) {
            throw new AdyenException('No skinCode set.');
        }
        if (! $merchantAccount) {
            throw new AdyenException('No merchantAccount set.');
        }
        if (! $shopperEmail) {
            throw new AdyenException('No shopperEmail set.');
        }
        if (! $shopperReference) {
            throw new AdyenException('No shopperReference set.');
        }
        ...
        if ($orderData) {
            $rv['orderData'] = base64_encode(gzencode($orderData));
        }
        if ($countryCode) {
            $rv['countryCode'] = $countryCode;
        }
        if ($shopperLocale) {
            $rv['shopperLocale'] = $shopperLocale;
        }
        if ($allowedMethods) {
            $rv['allowedMethods'] = $allowedMethods;
        }
        if ($blockedMethods) {
            $rv['blockedMethods'] = $blockedMethods;
        }
        if ($offset) {
            $rv['offset'] = $offset;
        }
        if ($shopperInteraction) {
            $rv['shopperInteraction'] = $shopperInteraction;
        }
    ...
    public function getSha256HPPParams()
    {
        ...
        foreach ($optionalParams as $key => $value) {
            ...
            } elseif ($key == 'issuerId' && $value) {
                $params[$key] = $value;
                $params['brandCode'] = $this->getAllowedMethods();
            } elseif ($value) {
                $params[$key] = $value;
            }
        }
    ...
    private function getMerchantSignature()
    {
        ...
        if (! $sharedSecret) {
            throw new AdyenException('No sharedSecret set.');
        }
    ...
    // ! Recurring Functions
    private function sendRecurringHTTPPost($request = [])
    {
    ...
    public function submitRecurringPayment($recurringDetailReference = null)
    {
        ...
        if (! $currencyCode) {
            throw new AdyenException('No Currency Code set.');
        }
